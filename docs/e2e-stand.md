...
- `refresh-golden-image-marker.sh` — без переливання даних відкочує чинний clean golden, fail-closed перевіряє exact SHA/checkout, оновлює лише image provenance marker і атомарно перезнімає всі чотири snapshots. Використовувати після code-only rebuild без міграцій або зміни fixtures.
